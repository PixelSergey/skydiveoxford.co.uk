...
const Intro = () => {
  const { data: introDays, isLoading } = useQuery({
    queryKey: ["intro-days"],
    queryFn: async () => {
      const { data, error } = await supabase.from("intro_days").select("*").order("display_order", { ascending: true });
            ...
            <div className="bg-card/80 backdrop-blur-sm rounded-lg p-6 border border-border/50">
              ...
              <div className="space-y-3">
                ...
                <p className="text-foreground leading-relaxed">
                  Welcome to the freshers' information session! Find out more about skydiving, how it all works, and try
                  on a real parachute here. Signups are optional but encouraged.
                </p>
                <div className="pt-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {isLoading ? (
                  ...
                  introDays.map((introDay) => (
                    <div
                      key={introDay.id}
                      className="text-center p-4 bg-background/50 rounded-lg border border-border/30"
                    >
                      <p className="text-xl text-foreground mb-4">{introDay.date}</p>
                      {introDay.state === "active" ? (
                        <a href={introDay.link} target="_blank" rel="noopener noreferrer">
                          <Button
                            variant="outline"
                            size="lg"
                            className="px-8 py-4 text-lg mb-3 whitespace-normal h-auto min-h-11"
                          >
                        ...
                        <Button
                          variant="outline"
                          size="lg"
                          className="px-8 py-4 text-lg opacity-50 cursor-not-allowed mb-3 whitespace-normal h-auto min-h-11"
                          disabled
                        >
                      ...
                      <p className="text-sm text-muted-foreground">Total places available: {introDay.total_places}</p>
                    </div>
                  ))
                ) : (
                  <div className="col-span-full text-center py-8">
            ...
            <div className="bg-card/80 backdrop-blur-sm rounded-lg p-6 border border-border/50">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-foreground">
                    Not at all! The intro day is designed for complete beginners. The Friday groundschool will teach you
                    everything you need to know about parachuting from the ground up.
                  </p>
                </div>
                <div>
